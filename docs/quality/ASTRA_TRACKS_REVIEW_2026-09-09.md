...
- Trace real consumers of `RoomTrackRepository.projections`: its five independently invalidated table Flows can briefly emit Fields before their Choice options. The new journey waits for observable commits then asserts a transactional snapshot. This avoids mistaking an intermediate projection for missing persisted data; it is not a production coherence fix or complete consumer audit.
