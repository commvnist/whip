...
- The smallest enlarged Entry validation frame presents errors and Add but its repeated Track introduction leaves inputs below the opening viewport. Existing Entry identity is repeated above editable Name. Wide definition/Entry forms span the full available width, leaving Field Edit and Scale controls far from their labels. Review these compositions with realistic authoring; acceptance of the nested Field/Date corrections does not close the main form design.
...
- Complete 641-line `domain/TrackCsv.kt` source read covers exact receipt/request identities, canonical fingerprints, normalized typed drafts, deterministic per-row UUIDs, header/mapping/required-value handling, strict quoted-cell parsing, and exact form boundaries. The real ViewModel preflights UTF-8/NUL, 5 MB, 5,000 rows, 100 columns and 100,000 preview cells before the full parser, so a parser-without-limits hypothesis does not describe the actual import path. CSV repository mutation/export assembly and the complete ViewModel/recovery/UI journeys still need review; this source read is not complete CSV acceptance.
