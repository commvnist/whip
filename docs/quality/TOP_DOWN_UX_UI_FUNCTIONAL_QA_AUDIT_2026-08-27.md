...
| SYS-015 | Section hierarchy | Shared `WhipSection`/`EditorSectionHeader` coexist with private Settings and raw Gym headings. | Low | Consolidate section tokens/components during touched-surface work. | Resolved: Settings and audited Gym headings now use `EditorSectionHeader`; guarded by `UiDesignArchitectureTest` (`a7d5d15`). |
...
| SYS-023 | Internal labels | Several surfaces render enum `.name` or raw names; this risks strings such as `WeightedMilestones` or group implementation names leaking to users. | Medium | Add/require explicit `uiLabel`; create a source contract that rejects unapproved user-facing `.name`. | Resolved for audited visible choices with explicit labels, localized weekdays, and source contracts (`a7d5d15`). |
...
| QA-008 | Repeated controls previously had ambiguous semantics and high-risk Gym workflows used positional action selectors. Lower-risk ordinal assertions remain for duplicated display content in other families. | Medium | Require field/lift-qualified semantics and named regions for repeated actions; permit positional assertions only when display order is itself under test. | High-risk Gym actions resolved in `IMP-20260902-023`; continuous display-assertion cleanup |
