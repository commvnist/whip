...
The rest increment (DEC/IMP/VER-20260910-018) keeps the specialized execution renderer and extracts duration precedence into one domain policy shared by ready/manual and automatic completion. The renderer names the source and owns override/reset presentation; the feature retains saved state, exact timer generations and Set history. Native phone/wide journeys and final regressions verify the 2:00-to-1:30 prescription correction, zero rest and reversible overrides. This is targeted reuse alongside builders; no general timer framework is justified.

The prescription increment (DEC/IMP/VER-20260910-017) applies the existing section-header role to a consequential form hierarchy: Set prescriptions come before ordinary guidance and bulk/conversion helpers, while the existing Training Max disclosure and advanced controls retain their meaning. The first Set moves 840 pixels upward at 100%, with load, the complete repetition range and Add Set visible. A full native author/recovery/warm-up/save/reopen/execute/finish/later-edit/History journey verifies exact values on phone/wide, alongside 209 Android regressions, seven wide methods and readiness. This demonstrates useful reuse without inventing a universal form API or coupling presentation to persistence. Longer Set lists still require scrolling to helpers; a future authoring-navigation contract should be justified by repeated actual workflows. FND-20260910-027 is subsequently verified in VER-20260910-018; shared wide screen policy remains active alongside builder adoption.
