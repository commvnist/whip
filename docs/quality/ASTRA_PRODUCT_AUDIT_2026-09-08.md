...
8. `FND-20260909-006`: real Android font scaling is now asserted through a shared rule in first-run, inspector, and nested-choice fixtures. The [32-fixture inventory](astra-dialog-font-review-2026-09-09.tsv) distinguishes four corrected cases from 28 pending investigations. Other passing outer-density dialog fixtures do not establish enlarged-text acceptance.
...
- First-run customization, optional preferences, pending/failure, actual 200% text/RTL, and setup-to-Home have explicit evidence. Window-theme and actual-font dialog additions expand the current catalog/matrix to 212 states; selected Home priority and independent dialog contrast are verified. Broader adaptive/accessibility and dialog-fixture font-scale review remain pending.
...
The Home, first-run, activity-theme, dialog-window, and inspector-header corrections are verified and ready for owner feedback. Whole-product area and per-surface review, additional-state discovery, remaining justified improvements, comprehensive final functional/static/build checks, full adaptive/accessibility/platform evidence, and final design acceptance remain incomplete. The goal remains active; continue with actual dialog font-scale verification and remaining shared/Home review, then every area in the matrix at equal depth.
