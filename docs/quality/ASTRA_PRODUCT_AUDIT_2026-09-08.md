...
8. `FND-20260909-006`: outer Compose density overrides do not prove dialog font scaling. First run now uses Android's actual setting and asserts rendered 2.0 scale; extend this evidence correction across remaining dialog accessibility coverage.
...
- First-run customization, optional preferences, pending/failure, actual 200% text/RTL, and setup-to-Home have explicit evidence. Six subsequent window-theme additions expand the current catalog/matrix to 207 states; selected Home priority and independent dialog contrast are verified. Broader adaptive/accessibility and dialog-fixture font-scale review remain pending.
