...
| Shared components and forms | ItemControlPatterns, ProductivityEditorComponents, EntityInspector, date/time/color/emoji/unit controls | In progress | All four dialog-window owners now share theme contrast; IMP/VER-20260909-006. Remaining component/form and actual font-scale review pending |
...
8. `FND-20260909-006`: all 32 entries in the [font inventory](astra-dialog-font-review-2026-09-09.tsv) now assert actual Android dialog text scale. Four were corrected in `VER-20260909-007`; the remaining 28 pass in `VER-20260909-008`. Font fidelity is distinct from visual acceptance; newly exposed layout findings remain open and native Search coverage still needs extension.
...
11. `FND-20260909-009` remains confirmed: short Task Templates and destructive recovery dialogs leave too little reading space; Area and backup previews initially expose no choices. Existing scroll tests pass, but initial discovery is not accepted. `FND-20260909-010` records a shared Task editor frame overlapping the status bar with IME open; establish settled-window behavior before deciding remediation.
...
- First-run customization, optional preferences, pending/failure, actual 200% text/RTL, and setup-to-Home have explicit evidence. Window-theme and actual-font additions expand the current catalog/matrix to 238 states; selected Home priority, independent dialog contrast, and all 32 inventoried font fixtures are verified. Broader adaptive/accessibility, short-dialog layout, and native-window coverage discovery remain pending.
...
The Home, first-run, activity-theme, dialog-window, inspector-header, and CSV recovery corrections have scoped verification. Whole-product area and per-surface review, additional-state discovery, remaining justified improvements, comprehensive final functional/static/build checks, full adaptive/accessibility/platform evidence, and final design acceptance remain incomplete. The goal remains active; continue with the open short-dialog and Task keyboard findings, native Search font coverage, and remaining shared/Home review, then every area in the matrix at equal depth.
