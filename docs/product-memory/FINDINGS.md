...
- Settings typed-value sub-resolution: Numeric, clock, time-zone, quiet-hour, retention, and other typed Settings now use bounded explicit drafts, strict validation, durable request receipts, failed-commit rollback/retry, lifecycle/conflict protection, and IME/large-text reachability. Adjacent Custom Unit/Health/choice families remain `FND-20260901-027` rather than being treated as implicitly resolved.
- Status: Partially resolved. Habit is verified in `IMP-20260831-009` / `VER-20260831-011`, Task in `IMP-20260831-010` / `VER-20260831-012`, Goal in `IMP-20260831-011` / `VER-20260831-013`, Gym permanent deletion in `IMP-20260901-012` / `VER-20260901-014`, Track definition/direct/CSV in `IMP-20260901-013` through `IMP-20260901-015`, the Gym active-session/quick-set subset in `IMP-20260901-016` / `VER-20260901-018`, and typed Settings in `IMP-20260901-017` / `VER-20260901-019`. Remaining lightweight Gym structure work is `FND-20260901-026`; adjacent Settings/Health ownership is `FND-20260901-027`; bounded shares and other secondary families still require separate risk-proportionate review.
...
- Status: Confirmed by the post-session Gym domain/UX audit; remediation pending as the next Gym integrity tranche.
...
- Status: Confirmed; typed-value and reminder-field follow-up behavior is resolved in `IMP-20260901-017`, while the remaining Custom Unit/Health/choice families require separate bounded tranches.
