...
- Observed source: `WhipApplication.initializeNormalRuntime` launches `PortableBackupManager.recoverInterruptedWrites` inside `runCatching` and discards the result. The manager does not update `PortableBackupState.lastError` on this path, and `cleanupStagingFiles` counts successful deletions while silently ignoring a provider that returns `false`. After persisted folder access is revoked, a provider goes offline, or an abandoned partial cannot be removed, startup can therefore leave Settings showing an earlier verified backup with no recovery warning. The folder-change transaction also commits the replacement URI before releasing the old grant; when that stale release throws, its catch path releases the new grant and reports failure despite the newly saved selection.
- Expected: Startup must remain available, preserve the configured folder and last verified receipt, and persist a visible, actionable warning when provider inspection or owned-partial cleanup fails. Choosing the folder again must clear the stale warning and permit recovery; forgetting a revoked folder must remain reliable. A recovery problem must never advance the last-success receipt or alter app records.
- Evidence: `PortableBackupManager.configureFolder` / `recoverInterruptedWrites` / `cleanupStagingFiles`, `WhipApplication.initializeNormalRuntime`, and the Data & Privacy portable-folder card. Focused fake-provider regressions reproduce a thrown offline/revoked listing, a `delete(false)` partial cleanup response and a stale old-grant release that breaks a committed replacement selection (`build/instrumentation-results-H53M0E`). A real SAF subfolder journey writes a verified backup, removes its persisted grant, makes the provider tree unavailable and reproduces the warning before successful reselection. Its first capture also exposed the raw platform `SecurityException`, including provider class, `ProcessRecord`, PID and UID, in user-visible copy.
- Root cause: Best-effort startup isolation was implemented at the application call site without a manager-owned durable failure receipt, while best-effort deletion did not distinguish “nothing to clean” from “owned partial still present.”
- Recommended solution: Keep startup non-fatal, but make the manager persist provider/recovery errors and report failed owned-partial cleanup. Preserve last verified metadata and automatic-backup configuration. Sanitize platform permission diagnostics while retaining their cause internally, give the visible card an explicit reconnect action, then verify failure and reselection through a real SAF folder.
- Related/status: FB-20260920-001, DEC/IMP/VER-20260920-004; Verified as an unreleased app change. Exact fake-provider baselines, real SAF loss/reselection, sanitized visual evidence and final readiness pass. Third-party/process-kill variants remain open audit work.
...
- Status: Released in Whip 0.3.72/code 78 under DEC/IMP/VER-20260915-001 and IMP/VER-20260915-002. The automatic two-second alarm test completes before its two-minute fallback without launching an Activity, allowed and denied exact-access branches pass, and five independent successors survive a late first occurrence. The owner phone now has precise-alarm access allowed and the exact signed APK installed in place.
...
### FND-20260910-040 — Equivalent page roles still have different heights and hierarchy

- Status/severity: Verified fix, P2; FB-20260910-007 / IMP, VER-20260911-001. Reproduced on unchanged 8f656b8b with five native catalog journeys at the owner's 360 dp width (1080×2520, 480 dpi, 100% text). Final phone/wide geometry and 25 distinct selected methods pass; released in 0.3.69/code 75. Owner appearance acceptance remains open.
- Observed: Task/Habit Today descriptions occupy one line while Goals/Tracks/Gym introductions wrap. Task History places its section selector above the heading. Gym page lists retain 12 dp spacing against other collections' 8 dp. Empty Track/Gym headings both render the shared 20 sp style; their descriptions are 14 sp beneath competing 16 sp page explanations, different copy lengths, 32 dp empty insets and differing list gaps. Task capture reserves another 8 dp even before its label floats.
- Cause: Shared text components alone do not define a consistent page composition. Prior geometry checks compared heading origins but accepted different following-content positions and did not compare empty-state rendered text styles. Previous 411 dp evidence was not the current owner viewport. Phone was locked during read-only inspection; reproduction uses synthetic emulator data, not owner content.
- Expected: Concise page context, one supporting-text role, consistent content/empty-state insets, controls below page identity, and floating-label space only when needed. Long authored text and accessibility text retain natural wrapping.
- Screenshot refinement: Material's label slot itself reserves another 8 dp above the visible outline even for blank unfocused input, inside semantics bounds. Removing only the outer allowance therefore leaves Task capture and Gym History search visually low. Reuse an inline-field builder that provides the idle name as a placeholder and composes a label when it floats; verify actual outline pixels as well as node bounds.
- Wide refinement: On the 1800×1200/density-160 viewport, switching Gym Workout to Progress changes the centered page measure from 720 to 1000 dp and moves its heading/actions 140 px. Tracks additionally changes its title inset from 12 to 20 dp between its list and Insights. Use one workspace measure and shared page insets while preserving real fold/support/browser pane ownership.
- Adjacent rendering regression: The compacted Gym Progress layout exposes the top edge of a chart-point chip at the bottom of the wide viewport. Its text child is clipped and the interactive node loses its name. The exact rejected hierarchy identifies `gym-chart-point`; give that existing control its full point label directly in semantics, preserving the same point action and calculations.
- Related: FND-20260910-039 / DEC-20260910-029 retain their verified historical scope; DEC-20260910-030 addresses the owner-rejected remaining appearance.

### FND-20260910-039 — Page headings reserve blank text and compound surrounding spacing

- Severity/status: P2 visual consistency; historical implementation checks pass in IMP/VER-20260910-030. Owner rejects remaining appearance under FB-20260910-007, addressed by FND-20260910-040 / IMP, VER-20260911-001.
...
- Confirmed observation: The 320×533 dp/actual-200% archive opening leaves barely any initial record space after global navigation, Track identity, inner destinations, status/Restore and tools. Older records become readable by scrolling (VER-20260909-019), but opening a populated history should present useful history sooner. The original `inline-keyboard/final-api26/tracks.history.archived.large.png` and its native bounds retain this evidence.
- Rejected review hypotheses: The original entry also alleged missing painted Close icons, clipped Area outlines, overlapping phone rows and touching wide labels. VER-20260909-020 independently decodes original pixels and compares native geometry: both complete X icons are present, Area margins are 24/43 pixels, all six titles occupy separate native intervals, wide Archived/Insights have 226 pixels between labels and inner destinations have 40 pixels. These were mistaken visual interpretations, not confirmed product defects. Preserve the correction rather than modifying working UI or capture timing to address them.
- Reproduction: Two ordinary/enlarged native journeys pass on each API 26/34 with a temporary four-frame diagnostic overlay. All API 26 state sequences and five of six API 34 sequences are byte-identical; the remaining difference is confined to system status icons. The earlier claim that an icon appeared later is explicitly withdrawn. Evidence: `artifacts/astra-audit/2026-09-09/track-frame-review/README.md`.
- Resolution: DEC-20260909-017 uses focused local detail below 600 dp in compact windows. First title/date are fully visible before scrolling, Back restores named global navigation, and active Add Entry plus local destinations remain available. The first small-device title starts 96 pixels earlier, with another 216 pixels of reading height recovered at 200% text. Complete Track authoring/source, wide-context/counts and active Entry-row composition remain under VER-20260909-016.
- Restored-state verification: The newly inventoried final phone capture rejects two unlabeled partially visible Entry actions. A temporary native hierarchy dump in `kN2ZKW` confirms clickable Edit/More nodes at `[744,1946][870,1973]` and `[870,1946][996,1973]` whose icon children have no visible description. Their labels lived on the scrolled-out icons. Move the unchanged Entry-qualified descriptions to the interactive buttons and make their icons decorative; retain the catalog's unlabeled-node rejection. This is a supported accessibility issue distinct from the previously rejected painting hypotheses.
- Related: FB-20260908-006, FND-20260909-017/018, VER-20260909-016/019/020, `docs/quality/ASTRA_TRACKS_REVIEW_2026-09-09.md`.
- Status: Verified for focused initial history reading and clipped Entry-action labels in IMP-20260909-020 / VER-20260909-021. Ordinary/enlarged journeys pass on API 26/34/37 with scoped personal review of 39 retained originals, final Tracks regressions and readiness checks. Previously rejected painting/collision hypotheses remain rejected. Broad Tracks and whole-product acceptance remain open.
