...
- Test only: no production behavior, schema, data epoch, backup format, release version or owner-phone operation changed. The exact method passes; the remaining Android tail and fresh complete gate are still required.
- Related/status: FB-20260920-001, FND-20260921-021, DEC-20260921-018, VER-20260921-023. Focused correction Verified; whole-product audit In progress.
...
- Compatibility/status: Released as 0.3.69/code 75 from clean pushed source fd2b239e98fa699052c0f8e945e0dcd38b3ad2b0 to the owner Samsung SM-F976W. Installed APK SHA-256 99559da76bb92c84b2bd5c37b29769d7cd56083311674a09ec84c88486175b53 matches the signed build; signer and first-install identity are preserved. Cold foreground launch, live process and bounded runtime checks pass. Room 46, epoch 6, backup 26, feature-owned state and record/history authority are unchanged. Receipt: artifacts/ui-alignment/2026-09-11/release-0.3.69/README.md. No exhaustive audit resumption; appearance awaits owner validation.
...
- Architecture: WhipWorkspaceLayout adds named Reading, Overview and Browser roles above the existing item families. The adaptive shell chooses pane ownership; item builders continue to own internal reading order and geometry; feature code retains state, scrolling, validation and commands. Remove the obsolete TrackOverviewSupportPane and five unused strings. No schema, backup, version, calculation or historical-data change.
- Verification/status: Verified under FND-20260910-030 / DEC/VER-20260910-021. Native bounds check the shared measure and header containment. Record actions/archive/history, running Habit timer recreation and explicit Settings Save pass pilots on phone/wide. The retained Tracks primary loading/error/empty-state guard replaces obsolete sidebar assertions. The complete-cycle fixture now waits for asynchronous Start Next to expose its existing Open Active Workout action before continuing exact history checks. Final 120 Android checks, six wide methods, 64 profile / 379 readiness JVM checks and lint/build pass. Thirty original images are personally reviewed; all 513 frozen inputs remain identical. Evidence: artifacts/astra-audit/2026-09-10/workspace-layout/README.md.
- Remaining scope: Dense analytics composition, independently filtered support-pane context, exact later/grouped Set navigation and substantive remaining whole-app review. This increment establishes shared screen responsibilities while preserving specialized Routine authoring. Delivery is recorded by Git history.
...
- Integrity: WhipApplication's existing synchronized generation advance removes old checkpoint files during reset/replace-restore. Checkpoint access shares that monitor and rejects obsolete owners, preventing an old editor from recreating deleted data. Recovery errors distinguish a still-open draft from unavailable authorship; stale or corrupt state cannot silently open a fresh Entry or substitute current saved values. Cleanup failures do not turn an already persisted Save into an apparent failure.
- Implementation: MainActivity lifecycle cleanup; root TrackEditorRouteHost and TrackScreens session ownership; private storage and guards in TrackEditorSessionViewModel; truthful adaptive failure presentation in TrackEditorRecoveryNotice. The existing editor state holders preserve their pure regression contracts. No Room/schema, historical-data, portable-backup or application-version change.
- Regression: TrackDraftSavedStateJourneyTest reproduces oversized active state and completed-session accumulation, then proves compact state and complete new-owner recovery. Seven TrackEditorCheckpointTest methods cover exact drafts/boundaries/raw inputs, cleanup/isolation, reuse, missing/incompatible/corrupt state and write failure. Two TrackDraftRecoveryUiTest methods cover four ordinary/actual-200% states. All 11 new methods join the permanent Tracks profile; RecoveryBoundaryIntegrationTest extends the real exclusive-reset contract.
- Verification: Final 64 JVM / 90 fresh Android neighbors pass; final state/checkpoint/recovery union passes 17 tests on each API 26/37. Earlier 13-test campaigns on API 26/34/37 additionally cover both complete ordinary/enlarged seven-type authoring journeys. Final API 34 process death restores unsaved authorship, preserves the Entry and all 600,008 note characters through Save, and removes its files. All 15 final originals have individual review and zero NAF nodes. Readiness passes 344 JVM tests, compilation/lint/packaging/static guards and both wrapper fixtures.
- Related/evidence: FND-20260909-027, DEC-20260909-023, VER-20260909-026, FB-20260908-006; `artifacts/astra-audit/2026-09-09/track-draft-recovery/README.md`. Baseline and development failures remain recorded. Current inventory: 630 JVM / 1,013 Android methods, 315 catalog/matrix states, 40 inventoried actual-font methods; the earlier 38-font claim is reconciled in VER-20260909-026.
- Status/limits: Verified for the Track editor transport/recovery seam. No physical-phone operation or release. Complete Track/app coverage remains open, including separate row-delete saved state, abnormal task-abandonment storage retention, large-data performance and Field/CSV journeys.
