...
Current baseline: 543 product tests—251 fast JVM tests and 292 Android
...
| Internal persistence and forward-compatible upgrades | checked-in pre-release schemas 1–9 and public schema 27, all forward to 28 | `WhipDatabaseMigrationTest` preserves schema-1 Tasks/Links/Contributions/Automations, schema-2 Tracks, schema-9 current-domain state, and schema-27 public entity-tag/Goal-snapshot data through schema 28; it also converts legacy Habit Skip/Excuse rows without retaining false measurements. `PersistentStorageE2ETest` closes and reopens a file-backed DB | signed public-0.3.7 APK upgrade retains a populated sentinel; `CoreFeatureJourneyE2ETest` recreates the Activity |
