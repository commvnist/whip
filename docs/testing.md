...
Current baseline: 1454 product tests—552 fast JVM tests and 902 Android
...
| Habit timer identity, canonical Duration conversion, monotonic elapsed display, reboot/restore review, and widget reachability | `HabitTimerDomainTest`, `HabitTimerClockTest`, `HabitTimerPresentationTest` | `HabitRepositoryTest`, `WhipDatabaseMigrationTest`, and `BackupRepositoryTest` cover idempotent requests, stale actions, unit snapshots, schema 41, and portable/private/merge profiles | `ProductivityCardDesignUiTest` verifies Start/Stop/Review semantics, 48 dp targets at 320dp/200% text, and editable Stop/Continue/Discard recovery; widget suites cover filtered active timers |
...
| Internal persistence and forward-compatible upgrades | checked-in pre-release schemas 1–9 and public schemas 27–39, all forward to 39 | `WhipDatabaseMigrationTest` preserves public-domain history through schema 39, including legacy routine-wave position, conservative 5/3/1 semantic backfill, explicit TM boundaries and provenance, decision history, session-level required-Main invalidation, immutable prescribed-set classification, orthogonal Goal archive state, stable closure identities, elapsed reset history, and private Track CSV commit receipts; it also deactivates retired rule configuration, dismisses pending legacy prompts, and prevents dormant Track references from blocking field or choice deletion. `PersistentStorageE2ETest` closes and reopens a file-backed DB | signed upgrade smoke retains populated data; `CoreFeatureJourneyE2ETest` recreates the Activity |
