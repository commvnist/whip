...
| Canonical local persistence and clean data boundary | checked-in schema 45 and epoch-boundary rules | `DataEpochBoundaryTest`, `DataEpochResetIntegrationTest`, and `PersistentStorageE2ETest` prove older local schemas never open directly, a person-confirmed fresh start creates schema 45, and a file-backed canonical database reopens cleanly | fresh-start confirmation and normal app recreation journeys |
