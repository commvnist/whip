...
Current baseline: 1171 product tests—486 fast JVM tests and 685 Android
...
| First-class Tracks, arbitrary typed Fields, exact authored-definition/removal boundaries, fractional-increment Scales, explicit sorting, 10,000-Entry calculations, typed filters, numeric summaries, paging, CSV, and choice replacement | `TrackDomainTest`, `TrackEditorStateTest`, `SortDirectionTest`, `EditorFeatureIntegrityTest` | `TrackDefinitionIntegrityTest`, `TrackRepositoryTest`, `TrackScaleMigrationTest`, and `BackupRepositoryTest` preserve Track definitions, Entries, choices, generated history, archived references, and dormant compatibility data while rejecting stale destructive review | `TrackDefinitionMutationUiTest` covers request ownership, conflict/copy recovery, missing/generation-stale Entry routes, dirty dialogs, exact review, TalkBack state, and 320dp/200% reachability; `InteractionControlUiTest`, `TrackWorkspaceUiTest`, and `AdaptiveWhipScreenTest` cover Scale interaction, navigation, RTL, large text, and adaptive layouts |
