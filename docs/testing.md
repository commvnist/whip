...
Current baseline: 556 product tests—252 fast JVM tests and 304 Android
...
| User-selected standard cards or compact list-style rows for Tasks, Habits, Goals, and Tracks | setting cause/effect contract and default behavior | `AppSettingsPersistenceTest` and `BackupRepositoryTest` preserve the preference | `ProductivityCardDesignUiTest` and `TrackWorkspaceUiTest` verify shorter rows, hidden secondary detail, and 48 dp actions; `SettingsBehaviorUiTest` verifies the Appearance toggle |
