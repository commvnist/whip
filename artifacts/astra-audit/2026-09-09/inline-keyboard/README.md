...
Known limits remain open: the smallest enlarged archive initially shows very little history; query and result can require scrolling separately; single-line Task input horizontally scrolls long drafts; the API 26 font lacks the boot emoji. Some frames also show crowded enlarged Track destinations, clipped Area outlines, apparent row overlap or missing painted action icons. These observations require settled runtime/source investigation under FND-20260909-019. Passing native bounds and journey tests do not accept those frames as comprehensive visual quality. Broad Tracks and whole-product acceptance remain open.
