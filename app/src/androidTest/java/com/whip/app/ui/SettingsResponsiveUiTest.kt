...
class SettingsResponsiveUiTest {
    ...
    fun destructiveCopyAndActionsRemainReachableInAShortLargeTextWindow() {
        ...
        val viewport = compose.onNodeWithTag("permanent-delete-content").getUnclippedBoundsInRoot()
        assertTrue("Short dialogs need room to read more than one line", viewport.bottom - viewport.top >= 180.dp)
        val cancelBefore = compose.onNodeWithText("Cancel").getUnclippedBoundsInRoot()
        val errorBounds = compose.onNodeWithText("Deletion could not finish. Your recovery marker is still active.")
            .performScrollTo()
            .assertIsDisplayed()
