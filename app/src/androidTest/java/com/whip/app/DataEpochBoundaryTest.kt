...
class DataEpochBoundaryTest {
    ...
    @Test fun trulyFreshInstallEstablishesCurrentMarkerWithoutReset() {
        ...
        try {
            assertEquals(DataEpochState.Current(2), DataEpochGate(context, name) { false }.evaluate())
            assertEquals("current:2", base.readText())
        } finally {
            base.delete()
    ...
    @Test fun markerStatePersistsAndInterruptedResetResumes() {
        ...
        try {
            ...
            assertEquals(DataEpochState.ResetInProgress(2), DataEpochGate(context, name) { true }.evaluate())
            gate.markCurrent()
            assertEquals(DataEpochState.Current(2), DataEpochGate(context, name) { true }.evaluate())
        } finally {
            base.delete()
            File(base.path + ".bak").delete()
        }
    ...
    @Test fun schemaFortyOneCannotBeOpenedDirectlyAsCanonicalSchemaFortyTwo() {
        val name = "schema-41-direct-open-${System.nanoTime()}"
        SQLiteDatabase.openOrCreateDatabase(context.getDatabasePath(name), null).use { legacy ->
            ...
            legacy.execSQL("PRAGMA user_version = 41")
        }
        ...
        try {
            ...
            assertTrue("Room must reject schema 41 without a migration", failure != null)
        } finally {
            database.close()
