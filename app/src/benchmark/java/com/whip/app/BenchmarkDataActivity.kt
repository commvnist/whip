...
class BenchmarkDataActivity : Activity() {
    ...
    private fun seedDenseHome(db: androidx.sqlite.db.SupportSQLiteDatabase, now: Long, today: Long) {
        ...
        db.execSQL("INSERT INTO measurement_definitions VALUES ('benchmark-habit-measurement','Benchmark check-ins','Integer','Count','count',0,1,0,$now,$now)")
        db.execSQL(
            "INSERT INTO habits (id,uuid,measurementId,name,notes,area,tagsCsv,icon,trackingMode,dimension,unitId,precision,comparison,targetMin,targetMax,targetPeriod,rollingDays,scheduleType,scheduleInterval,weekdaysMask,flexibleTimesPerWeek,startEpochDay,endType,endEpochDay,endValue,quickIncrement,quickActionsCsv,reminderMinutesCsv,weekdayReminderMinutesCsv,weekStart,timerStartedAtMillis,pinned,position,archived,paused,createdAtMillis,updatedAtMillis,sourceMeasurementId) VALUES (1,'benchmark-habit','benchmark-habit-measurement','Benchmark habit','','Benchmark','dense','✓','Count','Count','count',0,'AtLeast',10000,NULL,'Day',NULL,'Daily',1,0,NULL,$today,'Never',NULL,NULL,1,'','','','MONDAY',NULL,0,0,0,0,$now,$now,NULL)",
        )
        db.execSQL(
            "$FIVE_DIGIT_SEQUENCE INSERT INTO measurement_entries (id,measurementId,canonicalValue,enteredValue,enteredUnitId,status,timestampMillis,localEpochDay,zoneId,offsetSeconds,sourceType,sourceId,note,createdAtMillis,updatedAtMillis) " +
                "SELECT 'benchmark-habit-entry-'||x,'benchmark-habit-measurement',1,1,'count','Recorded',$now+x,$today,'UTC',0,'Habit','benchmark-habit-log-'||x,'',$now,$now FROM seq WHERE x < 10000",
        )
        db.execSQL(
            "$FIVE_DIGIT_SEQUENCE INSERT INTO habit_logs (uuid,habitId,value,canonicalValue,enteredUnitId,status,timestampMillis,localEpochDay,zoneId,offsetSeconds,note,sourceType,sourceId,measurementEntryId,createdAtMillis,updatedAtMillis) " +
    ...
    private fun seedLargeGraphs(db: androidx.sqlite.db.SupportSQLiteDatabase, now: Long, today: Long) {
        db.execSQL("INSERT INTO measurement_definitions VALUES ('benchmark-goal-measurement','Benchmark trend','Decimal','Unitless','unitless',1,1,0,$now,$now)")
        db.execSQL("INSERT INTO goals (id,uuid,measurementId,name,description,area,tagsCsv,icon,type,dimension,unitId,precision,baseline,targetMin,targetMax,direction,startEpochDay,deadlineEpochDay,aggregation,aggregationPeriod,rollingDays,paceType,consistencyPeriod,consistencyRequiredPeriods,elapsedStartMillis,elapsedDisplayUnit,reminderMinutes,status,pinned,position,createdAtMillis,updatedAtMillis) VALUES (1,'benchmark-goal','benchmark-goal-measurement','100k point goal','','Benchmark','dense','◎','OpenEndedTrend','Unitless','unitless',1,NULL,NULL,NULL,'Neutral',$today,NULL,'Latest','All',NULL,'None','Week',NULL,NULL,'Auto',NULL,'Active',0,0,$now,$now)")
        db.execSQL(
            "$FIVE_DIGIT_SEQUENCE INSERT INTO measurement_entries (id,measurementId,canonicalValue,enteredValue,enteredUnitId,status,timestampMillis,localEpochDay,zoneId,offsetSeconds,sourceType,sourceId,note,createdAtMillis,updatedAtMillis) " +
                "SELECT 'benchmark-goal-entry-'||x,'benchmark-goal-measurement',x%1000,x%1000,'unitless','Recorded',$now-(x*60000),$today-(x/10),'UTC',0,'Manual',NULL,'',$now,$now FROM seq WHERE x < 100000",
        )
